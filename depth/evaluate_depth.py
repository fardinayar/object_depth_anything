...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        encoder = DepthAnything_encoder.from_pretrained("LiheYoung/depth_anything_vits14")
        ...
        with torch.no_grad():
            for data in dataloader:
                ...
                features, h, w = encoder(input_color)
                output = depth_decoder(features, h, w)
                # TODO
                pred_disp = output[("disp", 0)] * scale.p
    ...
    gt_depths = np.load(gt_path, fix_imports=True, allow_pickle=True)["data"]
